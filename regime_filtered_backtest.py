...
Key Innovation: Stop trading entirely during market crashes

Use S&P500 as the market barometer. If S&P500 is in downtrend:
- Close all positions
- Stay in cash until uptrend resumes

This should dramatically reduce drawdowns at the cost of some CAGR.
"""
...
PARAMS = {
    # Market regime (S&P500 based)
    'market_ticker': 'SP500',
    'regime_sma': 200,             # Use 200-day SMA for regime
    'regime_buffer': 0.02,         # 2% buffer to avoid whipsaws
    ...
    # Position sizing
    'position_size': 0.07,         # Larger positions since fewer trades
    'max_positions': 10,
}
...
class RegimeFilteredBacktester:
    ...
    def _is_bull_market(self, date: datetime) -> bool:
        """Check if market (S&P500) is in uptrend"""
        ...
        # Bull if price is above SMA200 + buffer
        threshold = sma200 * (1 - PARAMS['regime_buffer'])
    ...
    def run_backtest(self, start_date='1980-01-01', end_date='2024-12-31'):
        print(f"\nRegime-Filtered backtest: {start_date} to {end_date}")
        print("=" * 70)
        ...
        for i, date in enumerate(trading_dates):
            ...
            is_bull = self._is_bull_market(date)

            # ===== REGIME FILTER: Close ALL if bear market =====
            if not is_bull and not in_cash:
                ...
                for ticker in list(self.positions.keys()):
                    if ticker in self.data and date in self.data[ticker].index:
                        ...
                        self.trades.append(Trade(
                            ...
                            pos.shares, pos.leverage, pnl, pnl_pct, days_held, "REGIME_EXIT"
                        ))
            ...
            # Normal processing only in bull market
            if is_bull:
                to_close = []
            ...
            if i % 1000 == 0:
                ...
                if years > 0:
                    ...
                    print(f"  {date.strftime('%Y-%m-%d')}: ${portfolio_value:,.0f} | CAGR: {cagr*100:.1f}% | DD: {dd:.1f}% | {regime}")
    ...
    def _generate_report(self):
        ...
        equity_df.to_csv('regime_filtered_equity.csv')
        return report


def main():
    print("\n" + "=" * 70)
    print("REGIME-FILTERED TREND-FOLLOWING BACKTEST")
    print("Stop trading during bear markets (S&P500 below 200-day SMA)")
    print("=" * 70)

    bt = RegimeFilteredBacktester('historical_data', 100000)
    r = bt.run_backtest()

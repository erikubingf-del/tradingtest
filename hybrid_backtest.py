...
Combines: Smart leverage + Crash protection + Momentum quality + Profit locking

Goal: Maximize CAGR/DD ratio (Calmar)
"""
...
PARAMS = {
    # Entry: Strong trend with acceleration
    'entry_mom_12m': 0.10,
    'entry_mom_6m': 0.05,
    'entry_mom_3m': 0.02,
    'entry_mom_1m': 0.01,
    ...
    # LEVERAGE: Very conservative - MAX 1.5x (key change!)
    'max_leverage': 1.5,           # Much lower than before
    'lev_confirm_days': 5,
    'lev_min_profit': 0.03,
    'lev_mom_accel': 0.005,        # Need momentum accelerating

    # MULTI-LAYER EXIT SYSTEM
    # Layer 1: Hard stop
    'stop_loss': -0.035,

    # Layer 2: Trailing stop (tightens with profit)
    'trail_base': 2.2,
    'trail_5pct': 1.8,
    'trail_10pct': 1.4,
    'trail_15pct': 1.0,

    # Layer 3: Momentum exits
    'exit_mom_reversal': -0.02,    # 1m momentum reversal
    'exit_mom_decel': -0.005,      # Momentum decelerating (for leveraged positions)

    # Layer 4: Daily crash protection
    'daily_crash_exit': -0.025,    # Exit on -2.5% daily move
...
class HybridBacktester:
    ...
    def _get_trailing_mult(self, profit_pct: float, leveraged: bool) -> float:
        if profit_pct >= 0.15:
            mult = PARAMS['trail_15pct']
        elif profit_pct >= 0.10:
            mult = PARAMS['trail_10pct']
        elif profit_pct >= 0.05:
            mult = PARAMS['trail_5pct']
        else:
    ...
    def run_backtest(self, start_date='1980-01-01', end_date='2024-12-31'):
        print(f"\nHybrid backtest: {start_date} to {end_date}")
        print("=" * 70)
        ...
        for i, date in enumerate(trading_dates):
            ...
            for ticker, pos in list(self.positions.items()):
                ...
                # ===== LAYER 4: Daily crash protection =====
                if not pd.isna(daily_ret) and daily_ret <= PARAMS['daily_crash_exit']:
                ...
                # Momentum deceleration (only for leveraged positions)
                mom_accel = row.get('Mom_Accel')
                if pos.leverage > 1 and not pd.isna(mom_accel):
                    if mom_accel < PARAMS['exit_mom_decel']:
                        # Don't exit, just deleverage
                        pos.leverage = 1.0

                # ===== UPDATE LEVERAGE =====
                if pos.days_profitable >= PARAMS['lev_confirm_days'] and pnl_pct >= PARAMS['lev_min_profit']:
                    if not pd.isna(mom_accel) and mom_accel > PARAMS['lev_mom_accel']:
                        pos.leverage = PARAMS['max_leverage']

                pos.prev_mom_1m = mom_1m if not pd.isna(mom_1m) else 0
    ...
    def _generate_report(self):
        ...
        equity_df.to_csv('hybrid_equity_curve.csv')
        return report


def main():
    print("\n" + "=" * 70)
    print("HYBRID TREND-FOLLOWING BACKTEST")
    print("Best of all approaches - MAX 1.5x leverage")
    print("=" * 70)

    bt = HybridBacktester('historical_data', 100000)
    r = bt.run_backtest()
